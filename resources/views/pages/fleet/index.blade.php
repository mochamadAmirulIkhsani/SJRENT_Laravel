...
@section('content')
    ...
    <section class="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 py-8">
        <div class="container mx-auto px-4 sm:px-6 lg:px-8">
            <form method="GET" action="{{ route('fleet.index') }}" class="flex flex-col md:flex-row gap-4">
                ...
                <!-- Submit Button -->
                <x-cta-button as="button" type="submit" variant="primary" size="sm">
                    Filter
                </x-cta-button>
                
                @if(request()->hasAny(['search', 'category']))
                <x-cta-button href="{{ route('fleet.index') }}" variant="neutral" size="sm">
                    Reset
                </x-cta-button>
                @endif
            </form>
    ...
    <section class="py-16 bg-blue-600 dark:bg-blue-900">
        <div class="container mx-auto px-4 sm:px-6 lg:px-8 text-center">
            ...
            <x-cta-button
                href="{{ $settings->getWhatsAppLink('Halo, saya ingin menanyakan ketersediaan motor.') }}"
                variant="success"
                size="md"
                target="_blank"
